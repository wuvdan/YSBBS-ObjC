//
//  CollectedUserListViewController.m
//  YSBBSApp
//
...
@interface CollectedUserListViewController ()<DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
...
@implementation CollectedUserListViewController
...
- (void)setupSubviews{
    ...
    [self.wdNavigationBar.centerButton setTitle:@"关注用户列表" forState:UIControlStateNormal];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CollectedUserModel *model = self.modelArray[indexPath.section];
    NSString *identifier = @"CollectedUserTableViewCell";
    CollectedUserTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[NSBundle mainBundle] loadNibNamed:@"CollectedUserTableViewCell" owner:self options:nil].firstObject;
    }
    cell.model = model;
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CollectedUserModel *model = self.modelArray[indexPath.section];
    controller.userId = model.followUserId;
...
- (NSAttributedString *)buttonTitleForEmptyDataSet:(UIScrollView *)scrollView forState:(UIControlState)state {
    NSAttributedString *att = [[NSAttributedString alloc] initWithString:@"您还没有关注任何用户..." attributes:@{NSForegroundColorAttributeName:kMainGrey,NSFontAttributeName:kFont(15)}];
    return att;
}
...
- (void)getListOfUserPostWithPageNo:(NSInteger)pageNo {
    ...
    [[BBSNetworkTool shareInstance] getCollectionUserListPostPageSize:@"10" pageNo:pageNo successBlock:^(id  _Nonnull obj) {
        if (pageNo == 1) {
            self.modelArray = [NSMutableArray array];
            for (NSDictionary *dic in obj[@"data"][@"list"]) {
                CollectedUserModel *model = [CollectedUserModel mj_objectWithKeyValues:dic];
